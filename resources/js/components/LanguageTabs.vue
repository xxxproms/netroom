<script setup lang="ts">
import { router, usePage } from '@inertiajs/vue3';
import { Languages } from '@lucide/vue';
import { computed, ref } from 'vue';
import { setLocale } from '@/i18n';
import { update } from '@/routes/locale';

const page = usePage();

const locales = computed(() => Object.entries(page.props.locales));
const current = ref(page.props.locale);

function select(locale: string): void {
    if (locale === current.value) {
        return;
    }

    // Switch the interface straight away, then remember the choice so every
    // later request (and anything the server renders) uses the same language.
    current.value = locale;
    setLocale(locale);

    router.patch(update().url, { locale }, { preserveScroll: true });
}
</script>

<template>
    <div
        class="inline-flex gap-1 rounded-lg bg-neutral-100 p-1 dark:bg-neutral-800"
    >
        <button
            v-for="[value, label] in locales"
            :key="value"
            @click="select(value)"
            :class="[
                'flex items-center rounded-md px-3.5 py-1.5 transition-colors',
                current === value
                    ? 'bg-white shadow-xs dark:bg-neutral-700 dark:text-neutral-100'
                    : 'text-neutral-500 hover:bg-neutral-200/60 hover:text-black dark:text-neutral-400 dark:hover:bg-neutral-700/60',
            ]"
        >
            <Languages class="-ml-1 h-4 w-4" />
            <span class="ml-1.5 text-sm">{{ label }}</span>
        </button>
    </div>
</template>
